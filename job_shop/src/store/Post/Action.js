...
import {
  ...
  DELETE_POST_FAILURE,
  DELETE_POST_SUCCESS,
  FETCH_COMPANY_POSTS_FAILURE,
  FETCH_COMPANY_POSTS_SUCCESS,
  FETCH_EMPLOYER_POSTS_FAILURE,
  FETCH_EMPLOYER_POSTS_SUCCESS,
  FETCH_MATCHED_POSTS_FAILURE,
  FETCH_MATCHED_POSTS_SUCCESS,
  FETCH_MATCHED_SEARCH_POSTS_FAILURE,
  FETCH_MATCHED_SEARCH_POSTS_SUCCESS,
  FETCH_POST_APPLICATIONS_FAILURE,
  FETCH_POST_APPLICATIONS_SUCCESS,
  ...
  SHOW_POSTS_AFTER_APPLY_SUCCESS,
  UPDATE_POST_FAILURE,
  UPDATE_POST_SUCCESS,
} from "./ActionType";
...
export const editPost = (postId, postData) => async (dispatch) => {
  try {
    const { data } = await api.put(
      `/api/employer/updatePost/${postId}`,
      postData
    );
...
export const findFilteredPosts = (postSearch) => async (dispatch) => {
  try {
    console.log(postSearch);
    const { data } = await api.post(`/api/Post/postSearch`, postSearch);
    console.log("returned Posts From Search : ", data);
    dispatch({ type: FETCH_MATCHED_SEARCH_POSTS_SUCCESS, payload: data });
  } catch (error) {
    dispatch({
      type: FETCH_MATCHED_SEARCH_POSTS_FAILURE,
...
export const deleteApplication = (applicationsId) => async (dispatch) => {
  try {
    const { data } = await api.delete(
      `/api/Post/deleteApplication/${applicationsId}`
    );
...
export const acceptApplication = (applicationsId) => async (dispatch) => {
  try {
    const { data } = await api.put(
      `/api/Post/acceptApplication/${applicationsId}`
    );
...
export const applyForPost = (applicationData) => async (dispatch) => {
  try {
    ...
    console.log(
      "Response for apply : Remained Skills : ",
      data.remainedSkills,
      " Remained Qual :",
      data.remainedQualifications
    );
