...
import {GET_EMPLOYER_POSTS_SUCCESS, GET_EMPLOYER_POSTS_FAILURE, GET_EMPLOYER_FIELDS_SUCCESS, GET_EMPLOYER_FIELDS_FAILURE, GET_EMPLOYER_PROFILE_SUCCESS, GET_EMPLOYER_PROFILE_FAILURE, UPDATE_EMPLOYER_PROFILE_SUCCESS, UPDATE_EMPLOYER_PROFILE_FAILURE, UPDATE_EMPLOYER_CONTACTS_SUCCESS, UPDATE_EMPLOYER_CONTACTS_FAILURE } from "./ActionType"
...
export const updateEmployerProfile=(formData)=>async(dispatch)=>
    ...
    export const updateEmployerContacts=(contacts)=>async(dispatch)=>
        {
        try {
            const {data}=await api.put(`/api/user/updateContacts`,contacts);
            dispatch({type:UPDATE_EMPLOYER_CONTACTS_SUCCESS,payload:data});
        } catch (error) {
            dispatch({type:UPDATE_EMPLOYER_CONTACTS_FAILURE,payload:error.message});
        }
        }
