...
export default function ApplyModal({
  ...
  const [jobSeekerSkills, setJobSeekerSkills] = React.useState([
    { skillName: "", skillId: 0 },
  ]);
  const [jobSeekerQualifications, setJobSeekerQualifications] = React.useState([
    { qualificationName: "", qualificationId: 0 },
  ]);
  const [openShowApplicationResultsModal, setOpenShowApplicationResultsModal] =
    React.useState(false);
  const handleOpenShowApplicationResultsModal = () =>
    setOpenShowApplicationResultsModal(true);
  const handleCloseShowApplicationResultsModal = () =>
    setOpenShowApplicationResultsModal(false);
  ...
  React.useEffect(() => {
    if (openApplyModal) {
      ...
      formik.setFieldValue(
        "applicationQualifications",
        ListOfObjectOfUpdatedQualifications
      );
      ...
      const ListOfObjectsOfSkills = updateSkillsList.map((skill) => ({
  ...
  const handleAddSkill = (skill, skillId) => {
    if (!selectedSkills.some((skill) => skill.skillId === skillId)) {
      const updatedSkills = [...selectedSkills, { skill, skillId }];
      ...
      const ListOfObjectOfUpdatedSkills = updatedSkills.map((skill) => ({
        jobSeekerSkillId: skill.skillId,
      }));
  ...
  const handleAddQualification = (qualification, qualificationId) => {
    if (
      ...
      // Map the qualificationIds of selectedQualifications to an array for faster lookup
      const selectedQualificationIds = updatedQualifications.map(
        (qual) => qual.qualificationId
      );
  ...
  const handleRemoveQualification = (
    ...
    qualificationIdToRemove
  ) => {
    const updatedQualifications = selectedQualifications.filter(
      (qualification) =>
        qualification.qualificationId !== qualificationIdToRemove
    );
            ...
            <form onSubmit={formik.handleSubmit}>
              <Grid container direction="column" spacing={2}>
                      ...
                      ? selectedSkills.map((skill, index) => (
                          ...
                          >
                            ...
                            {!skills.includes(skill.skill) && (
                              <IconButton
                                onClick={() =>
                                  handleRemoveSkill(skill.skill, skill.skillId)
                                }
                                aria-label="delete"
                                size="small"
                              >
                      ...
                      <div>
                        {jobSeekerSkills.map((skill, index) => (
                          <Button
                            ...
                            variant="outlined"
                            onClick={() =>
                              handleAddSkill(skill.skillName, skill.skillId)
                            }
                            style={{
                              marginBottom: "0.5rem",
                              marginRight: "0.5rem",
                ...
                <Grid item xs={12}>
                  ...
                  <div
                    className="selected-qualifications-container"
                    style={{ marginLeft: "1rem", fontSize: "1.25rem" }}
                  >
                    {selectedQualifications.length > 0
                      ? selectedQualifications.map((qualification, index) => (
                          <div
                            key={index}
                            className="selected-qualification"
                            style={{
                              display: "flex",
                              alignItems: "center",
                              marginBottom: "0.5rem",
                ...
                <Grid item xs={12}>
                  <div
                    ...
                    style={{ maxHeight: "200px", overflowY: "auto" }}
                  >
                    {jobSeekerQualifications &&
                      jobSeekerQualifications.length > 0 && (
                        <div>
                          {jobSeekerQualifications.map((qual, index) => (
                            <Button
                              key={index}
                              variant="outlined"
                              onClick={() =>
